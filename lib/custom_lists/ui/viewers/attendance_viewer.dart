...
class _AttendanceViewerState extends ConsumerState<AttendanceViewer> {
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, dynamic> attendance = Map<String, dynamic>.from(
      widget.content['attendance'] as Map? ?? const {},
    );
    final List<String> roster =
        (widget.content['roster'] as List?)?.cast<String>() ?? const <String>[];
    ...
    final List<UserEntity> users =
        usersAsync.asData?.value ?? const <UserEntity>[];
  ...
  final monthLabel = _visibleMonth.isEmpty ? _monthNameFromDate(_startDay) : _visibleMonth;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          // Left: Names column with its own vertical scroll
          SizedBox(
            width: nameColWidth,
            child: Column(
              children: [
          ...
          // Right: One horizontal scrollable that wraps header + grid to guarantee sync
          Expanded(
            child: LayoutBuilder(
              builder: (context, constraints) {
                ...
                return SingleChildScrollView(
                  ...
                  child: SizedBox(
                    ...
                    child: Column(
                      children: [
                        // Days header inside the same horizontal scroll
                        Container(
                          height: headingHeight,
                          color: theme.colorScheme.surfaceVariant.withOpacity(0.25),
                          child: Row(
                        ...
                        const Divider(height: 1),
                        // Grid body with its own vertical scroll, aligned by fixed column widths
                        SizedBox(
                                        ...
                                        child: Center(
                                          child: Transform.scale(
                                            ...
                                            child: _InlineCell(
                                              ...
                                              onPersist: (nextAttendance) async {
                                                final nextContent = Map<String, dynamic>.from(widget.content);
                                                nextContent['attendance'] = nextAttendance;
                                                await ref.read(customListsServiceProvider).setListContent(
                                                  widget.listId,
                                                  nextContent,
                                                );
                                              },
...
class _InlineCell extends StatefulWidget {
  ...
  final Future<void> Function(Map<String, dynamic> nextAttendance) onPersist;
  const _InlineCell({
    required this.listId,
